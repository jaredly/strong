...
Strong.prototype = {
  ...
  lookup: function(locale, path, key, options) {
    var attempts = [];
    ...
    for (var i = 0; i < locale.length; i++) {
      ...
      while (context.length > 1 && !exists(template)) {
        template = this.back.navigate(context.join('.'), options);
  ...
  translate: function(key, options, navOptions) {
    // Provide an empty options object if none was given.
    if (options == null) options = {};

    var locale = this.effective_locale(options);
    var template = this.lookup(locale, options.view_path, key, navOptions);
  ...
  effective_locale: function(options) {
  ...
  decorator: function(engine, appname) {
    var self = this;
    return {
      compile: function (template, options) {
        ...
        var translation_helper = self.translationHelper(view_path, options);
  ...
  /**
   * Create the `i18n` function for a particular view
   *
   * @api private
   */

  translationHelper: function (view_path, view_options) {
    var self = this;
